...
export function listarProductos() {
    ...
    db.inventario.forEach(producto => {
        ...
        if (producto.ingredientes && producto.ingredientes.length > 0) {
            ...
            console.log(`   ${etiqueta}: ${producto.ingredientes.join(", ")} - Stock: ${producto.stock}`);
        }
    });
    console.log("------------------------------------\n");
}

function agregarProducto(nombre, categoria, precio, ingredientes, stock) {
    const db = leerDB();
    const id_producto = db.inventario.length > 0 ? Math.max(...db.inventario.map(p => p.id_producto)) + 1 : 1;
    
    db.inventario.push({ id_producto, nombre, categoria, precio, ingredientes, stock });
    guardarDB(db);
    ...
    while (!salir) {
        ...
        switch (opcion.trim()) {
            ...
            case '2':
                ...
                const stockStr = await rl.question("Stock: ");
                
                const precio = parseFloat(precioStr);
                const ingredientes = ingredientesStr.split(',').map(i => i.trim());
                const stock = parseInt(stockStr);
                agregarProducto(nombre, categoria, precio, ingredientes, stock);
                await rl.question("\n[ENTER] PARA CONTINUAR");
                break;
            case '3':
                listarProductos();
